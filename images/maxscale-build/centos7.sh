...
#解压
tar -Jxf node-v26.8.1-linux-x64-glibc-217.tar.xz -C /opt/
#软链接（全局生效）
ln -sf /opt/node-v26.8.1-linux-x64-glibc-217/bin/node /usr/local/bin/node
ln -sf /opt/node-v26.8.1-linux-x64-glibc-217/bin/npm /usr/local/bin/npm
ln -sf /opt/node-v26.8.1-linux-x64-glibc-217/bin/npx /usr/local/bin/npx
#验证
node -v
npm -v
echo "CentOS 7 maxscale-build environment setup completed successfully!"
